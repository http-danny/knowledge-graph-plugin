...
## Launching under Gemini CLI

These tools are served by the `kg` MCP server bundled in this extension, under the
read-only **`safe`** tool profile (raw-Cypher tools are not exposed).

- Run `gemini` from your **kg project root** (the directory holding `kg.config.json`); the
  bundle pins `KG_CONFIG_PATH` to `<workspace>/kg.config.json`.
- Launched from a subdirectory and tools fail with "kg.config.json not found"? Re-run from
  the project root, or pin an absolute path: `kg mcp-config --agent gemini --write` (writes a
  local `.gemini/settings.json` that overrides this bundle).
